...
import { Roles } from 'src/conmmon/auth.decorator';
...
export class AdminController {
  ...
  @ApiOperation({
    ...
    description: 'Fetches the count of projects for each status category including "Rejected", "In Progress", "On Going", and "Done".'
  })
  ...
  @ApiOperation({
    ...
    description: 'Fetches users based on the role. Input "All Roles" to display all users, "admin" for admin users, "talent" for talent users, or "customer" for customer users.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Updates user information based on the provided ID and details.'
  })
  async updateUser(
  ...
  @ApiOperation({
    ...
    description: 'Soft deletes a user by their ID, marking the user as deleted but not physically removing them from the database.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Updates project information based on the provided ID and details.'
  })
  async updateProject(
    @Param('id', ParseIntPipe) id: number,
  ...
  @ApiOperation({
    ...
    description: 'Soft deletes a project by its ID, marking the project as deleted but not physically removing it from the database.'
  })
  async softDeleteProject(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      throw new HttpException('Failed to soft delete project', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async findAlluser() {
    try {
      ...
      throw new HttpException('Failed to fetch project talents', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Fetches talents associated with a specific project by its ID.'
  })
  async findProjectTalent(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      throw new HttpException('Failed to fetch project talent', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Soft deletes a project talent by its ID, marking the talent as deleted but not physically removing it from the database.'
  })
  async softDeleteProjectTalent(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      throw new HttpException('Failed to soft delete project talent', HttpStatus.BAD_REQUEST);
    }
  }
}
